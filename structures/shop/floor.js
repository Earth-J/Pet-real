...
const shopFloor = async (client, interaction, msg, item) => {
    if (!interaction && !interaction.channel) throw new Error('Channel is inaccessible.');
        ...
        const buildNavRow = () => new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('shop_floor_first').setLabel('⏮️').setStyle(ButtonStyle.Secondary).setDisabled(page === 0 || totalPages <= 1),
            new ButtonBuilder().setCustomId('shop_floor_prev').setLabel('◀️').setStyle(ButtonStyle.Secondary).setDisabled(page === 0 || totalPages <= 1),
            new ButtonBuilder().setCustomId('shop_floor_close').setLabel('ปิด').setStyle(ButtonStyle.Danger),
            new ButtonBuilder().setCustomId('shop_floor_next').setLabel('▶️').setStyle(ButtonStyle.Secondary).setDisabled(page >= totalPages - 1 || totalPages <= 1),
            new ButtonBuilder().setCustomId('shop_floor_last').setLabel('⏭️').setStyle(ButtonStyle.Secondary).setDisabled(page >= totalPages - 1 || totalPages <= 1)
        );
