...
function checkCooldown(userId) {
  const now = Date.now();
  const lastUsed = walkCooldowns.get(userId);
  
  if (lastUsed && (now - lastUsed) < WALK_COOLDOWN) {
    const remaining = Math.ceil((WALK_COOLDOWN - (now - lastUsed)) / 1000);
    return remaining;
  }
...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (cooldownRemaining > 0) {
      return interaction.editReply(`⏰ คุณต้องรอ **${cooldownRemaining} วินาที** ก่อนที่จะพาสัตว์เลี้ยงเดินเล่นได้อีกครั้ง`);
    }
    ...
    await withUserLock(interaction.guild.id, interaction.user.id, async () => {
      ...
      // อัปเดต cooldown
      walkCooldowns.set(interaction.user.id, Date.now());
