...
module.exports = {
  name: ["สัตว์เลี้ยง", "อาบน้ำ"],
  description: "อาบน้ำสัตว์เลี้ยง (คูลดาวน์ 3 นาที)",
  category: "Pet",
  run: async (client, interaction) => {
    ...
    if (cooldownRemaining > 0) {
      return interaction.editReply(`⏰ ดูเหมือนสัตว์เลี้ยงของคุณจะยังไม่อยากอาบน้ำ รอ **${cooldownRemaining} วินาที** `);
    }
    ...
    await withUserLock(interaction.guild.id, interaction.user.id, async () => {
      ...
      .setAuthor({ name: `อาบน้ำให้ ${pet.name}`, iconURL: "https://cdn.jsdelivr.net/gh/Earth-J/cdn-files@main/icon-sleep.png" })
        .setColor('#e8f093')
        .setThumbnail('https://cdn.jsdelivr.net/gh/Earth-J/cdn-files@main/thumbnail/clean.png')
        .setTimestamp();
      ...
      // EXP รวมในบล็อคเดียวกัน (ใส่ fallback กันค่าว่าง)
      const expNow = Number.isFinite(result?.exp) ? result.exp : 0;
      const expNext = Number.isFinite(result?.nextexp) ? result.nextexp : 0;
      const expDelta = Number.isFinite(result?.expGain) ? result.expGain : 0;
      const leveledUpText = result?.leveledUp ? `\n**เลเวลอัป!** → เลเวล ${result.level} 🎉` : '';
      lines.push(`<:exp:1424394377555607592> **EXP :** ${fmt(expNow)}/${fmt(expNext)} (+${fmt(expDelta)})${leveledUpText}`);
